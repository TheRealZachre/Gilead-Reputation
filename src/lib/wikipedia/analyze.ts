...
async function getNarrativeSignals(): Promise<string[]> {
  // The reputation app does not sync social posts, so there are no
  // narrative signals to cross-reference against Wikipedia coverage.
  return [];
}
...
function buildPageSnapshot(
  ...
  const mentionsCurrent = mentionsTerm(textBlob, [config.currentBrand]);
  ...
  if (snapshot.wordCount < 400) {
    findings.push(
      makeFinding({
        ...
        howToFix:
          "Expand Career and leadership sections with cited milestones: appointment, major regulatory wins, and board roles — each tied to independent journalism or filings.",
        impact: "medium",
        effort: "medium",
      })
    );
...
function buildCrossFindings(
  ...
  if (!mentionsTerm(ceo.sections.join(" "), [companyName])) {
    findings.push(
      makeFinding({
